...
@export var afflux_camera_position := Vector3(6.0, 8.0, 6.0)
@export var afflux_camera_rotation_y := 0.0
@export var afflux_camera_zoom_size := 12.0
...
func start_named_sequence(sequence_name: StringName) -> bool:
	match sequence_name:
		&"afflux_window":
			return start_afflux_sequence()
		_:
			push_warning("Séquence inconnue : %s" % sequence_name)
			return false

func start_afflux_sequence() -> bool:
	if not start_sequence():
		return false

	if camera_pivot != null and camera_pivot.has_method("set_scripted_view"):
		camera_pivot.set_scripted_view(afflux_camera_position, afflux_camera_rotation_y, afflux_camera_zoom_size)

	var started_sequence_id := _sequence_id
	call_deferred("_run_afflux_sequence", started_sequence_id)
	return true

func _run_afflux_sequence(started_sequence_id: int) -> void:
